(function($) {
	...
	dropdown.buildCartItem = function($elem, data) {
		...
		for (var i = 0, len = data.length; i < len; i++) {
			html += '<li class="cart-item"><a href="###" target="_blank" class="cart-item-pic fl"><img src="' + data[i].pic +
				'" alt="" /></a><div class="fl"><p class="cart-item-name text-ellipsis"><a href="###" target="_blank" class="link">' +
				data[i].name + '</a></p><p class="cart-item-price"><strong>￥' + data[i].price + ' x ' + data[i].num +
	...
	dropdown.updateCart = function($elem, data) {
		...
		for (var i = 0, len = dataNum; i < len; i++) {
			totalNum += +data[i].num;
	...
	dropdown.createCategoryDetails = function($elem, data) {
		...
		for (var i = 0, len = data.length; i < len; i++) {
			...
			for (var j = 0, lens = data[i].items.length; j < lens; j++) {
	...
	floor.buildFloorHead = function(floorData) {
		...
		html += '<h2 class="floor-title fl"><span class="floor-title-num">' + floorData.num +
			'F</span><span class="floor-title-text">' + floorData.text + '</span></h2>';
		html += '<ul class="tab-item-wrap fr">';
		for (var i = 0, len = floorData.tabs.length; i < len; i++) {
	...
	floor.buildFloorBody = function(floorData) {
		...
		for (var i = 0, len = floorData.items.length; i < len; i++) {
			html += '<ul class="tab-panel">';
			for (var j = 0, lens = floorData.items[i].length; j < lens; j++) {
				html += '<li class="floor-item fl">';
				html +=
					'<p class="floor-item-pic"><a href="###" target="_blank"><img src="img/floor/loading.gif" class="floor-img" data-src="img/floor/' +
					floorData.num + '/' + (i + 1) + '/' + (j + 1) + '.png" alt="" /></a></p>';
				html += '<p class="floor-item-name"><a href="###" target="_blank" class="link">' + floorData.items[i][j].name +
					'</a></p>';
	...
	foot.timeToShow = function() {
		if (lazyLoad.isVisible(foot.$helper.offsetTop, foot.$helper.height) || lazyLoad.isVisible(foot.$footer.offsetTop,
				foot.$footer.height)) {
